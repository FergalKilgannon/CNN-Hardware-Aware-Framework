...
class forwardPass(hardware):
    ...
    def forward_pass(self, data, batch):
        ...
        x = self.conv2d(x, batch, self.model.conv1.module, s_in, self.idim, 3, 32, self.knl)

        # ReLU 1
        x = self.relu6(x, 0, 6, self.model.relu1.module)

        # Maxpool layer 1
        x = self.maxpool2d(x, batch, self.idim - self.knl + 1, 32, 2)
        ...
        x = self.conv2d(x, batch, self.model.conv2.module, sin_conv2, (self.idim-self.knl+1)/2, 32, 64, self.knl)

        # ReLU 2
        x = self.relu6(x, 0, 6, self.model.relu2.module)

        # Maxpool layer 2
        x = self.maxpool2d(x, batch, ((self.idim - self.knl+1)/2) - self.knl + 1, 64, 2)
